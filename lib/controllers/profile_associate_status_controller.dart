...
class BdVProfileAssociateStatusController extends ChangeNotifier {
  ...
  Future<void> loadProfileAssociateStatus( String id, String hld ) async {
    ...
    // ==========================================
  Future<List<AssociateStatusModel>> loadAvailableAssociateStatus( String hld ) async {
    try {
      loadingNotifier.value = true;
      errorNotifier.value = null;
      ...
      availableAssociateStatus.value = resposta.map(
        ( item ) => AssociateStatusModel.fromMap( item )
      ).toList();

      return availableAssociateStatus.value;
